...
***There are `r nrow(dennys)` rows and `r ncol(dennys)` columns.  Each row represents the data for one dennys resturant.  The variables are `r names(dennys)`.***    
...
***La Quinta hotels are mostly located in the Americas, but also have locations in other parts of the world.  Dennys are located in the U.S.***
...
laquinta %>%
  mutate(country = case_when(
    ...
    state == "FM"            ~ "Honduras" # fill in the rest
  ))
```
